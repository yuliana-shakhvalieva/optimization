...
def grad_finite_diff(func, x, eps=1e-8):
    ...
    """
    n = x.shape[0]
    return np.apply_along_axis(lambda z: (func(x + z * eps) - func(x)) / eps, 1, np.eye(n))
...
def hess_finite_diff(func, x, eps=1e-5):
    ...
    n = x.shape[0]
    E = np.eye(n)
    return np.array([[(func(x + e_i * eps + e_j * eps) - func(x + e_i * eps) - func(x + e_j * eps) + func(x)) / eps ** 2
                      for e_i in E] for e_j in E])
...
#
# test = 1
# for func, finite_func in zip([oracle.grad, oracle.hess], [grad_finite_diff, hess_finite_diff]):
#     for i in range(5):
#         x = np.random.rand(4)
#         print(f'Test: {test} {np.allclose(func(x), finite_func(oracle.func, x), rtol=1e-1, atol=1e-1)}.')
#         test += 1
